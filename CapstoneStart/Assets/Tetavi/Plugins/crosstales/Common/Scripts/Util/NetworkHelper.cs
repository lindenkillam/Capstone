...
namespace Crosstales.Common.Util
{
   ...
   public abstract class NetworkHelper
   {
      ...
      public static string ValidURLFromFilePath(string path)
      {
         ...
         {
            if (!isValidURL(path))
               return BaseConstants.PREFIX_FILE + System.Uri.EscapeUriString(FileHelper.ValidateFile(path).Replace('\\', '/'));

            return System.Uri.EscapeUriString(FileHelper.ValidateFile(path).Replace('\\', '/'));
         }
